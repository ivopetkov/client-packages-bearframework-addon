...
class ClientPackages
{
     ...
     * @return array|null
     */
    private function getDOMDetails(string $html): ?array
    {
        $result = null;
        if (strpos($html, 'client-packages') !== false) {
            ...
            if ($splitIndex !== false) {
                ...
                $dom->loadHTML(substr($html, 0, $splitIndex) . '</head><body></body></html>', HTML5DOMDocument::ALLOW_DUPLICATE_IDS);
                ...
                if ($head !== null) {
                    ...
                    return [$splitIndex, $dom, $head, $libraryInsertTarget, $packagesToEmbed, $packagesToPrepare, $elementsToRemove];
                }
            }
        }
        return $result;
    ...
    {
        $domDetails = $this->getDOMDetails($html);
        if ($domDetails !== null) {
            list($splitIndex, $dom, $head, $libraryInsertTarget, $packagesToEmbed, $packagesToPrepare, $elementsToRemove) = $domDetails;
            $hasChange = false;
            if ($libraryInsertTarget !== null) {
     ...
     * @return array
     */
    public function getResourcesURLs(string $html): array
    {
        $domDetails = $this->getDOMDetails($html);
        if ($domDetails !== null) {
            list($splitIndex, $dom, $head, $libraryInsertTarget, $packagesToEmbed, $packagesToPrepare, $elementsToRemove) = $domDetails;
            $resources = Utilities::getResources($packagesToEmbed, $packagesToPrepare);
            return array_merge($resources['jsFiles'], $resources['jsFilesAsync'], $resources['cssFiles']);
        }
        return [];
    }
}
